...
class Unique implements ValidationRule, ValidatorAwareRule
{
    /**
    ...
    {
        if (! $this->validator || ! method_exists($this->validator, 'validateUnique')) {
            throw new RuntimeException('Unique rule requires validator implementation with validateUnique method.');
        }

        if (! $this->validator->validateUnique($attribute, $value, $parameters ?? [])) {
            $fail($attribute);
        }
    }
