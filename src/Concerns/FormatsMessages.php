...
trait FormatsMessages
{
    ...
    protected function getFromLocalArray($attribute, $lowerRule, $source = null)
    {
        ...
        if ($this->getAttributeType($attribute) !== 'file') {
            $shortRule = "{$attribute}.".Str::snake(str_contains($lowerRule, '\\') ? substr($lowerRule, strrpos($lowerRule, '\\') + 1) : $lowerRule);
    ...
    public function makeReplacements($message, $attribute, $rule, $parameters)
    {
        ...
        if (isset($this->replacers[Str::snake($rule)])) {
            ...
            $snakeRule = Str::studly($rule);
            if (method_exists($this, $replacer = "replace{$snakeRule}")){
